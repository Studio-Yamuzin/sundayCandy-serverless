    ...
    try{
      const mappedConnection = await Promise.all(connections.map(async (connection): Promise<RoomInfo> => {
        ...
        if(connection.recentReadMessage){
          unReadCount = recentMessage.findIndex((message) => message.SK === connection.recentReadMessage);
          if(unReadCount !== 0){
            unReadCount += 1;
          }
        }
      ...
      return mappedConnection.sort((aRoom, bRoom) => new Date(aRoom.recentMessage.timeStamp).getDate() - new Date(bRoom.recentMessage.timeStamp).getDate());
    }catch(error){
      console.log(error);
    }
